...
const insertMovieDirectors = (
  ...
  return (
    ...
    directors.map(director => `('${movieId}', '${directorRows.find(d => director === d.full_name)!.id}')`).join(",")
  );
};
...
const insertMovieKeywords = (
  ...
  return (
    ...
    keywords.map(keyword => `('${movieId}', '${keywordRows.find(k => keyword === k.keyword)!.id}')`).join(",")
  );
};
...
const insertMovieProductionCompanies = (
  ...
  return (
    ...
    productionCompanies.map(company => `('${movieId}', '${productionCompanyRows.find(pc => company === pc.company_name)!.id}')`).join(",")  );
};
